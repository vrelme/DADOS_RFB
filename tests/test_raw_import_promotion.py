...
def _repository_for_swap_test(table_exists):
    ...
    repo._audit_monitored_fields = lambda table_name: repo.audit_calls.append(table_name)
...
def test_rename_swap_skips_audit_when_final_table_does_not_exist(monkeypatch):
    ...
    assert "comparacao/auditoria foi ignorada" in repo.control_rows[0][2]


def test_rename_swap_audits_when_final_table_exists(monkeypatch):
    table = SimpleNamespace(table_name="empresa")
    monkeypatch.setattr(raw_import_promotion, "RFB_TABLES", [table])
    ...
    assert repo.audit_calls == ["empresa"]
    assert repo.swap_calls == ["empresa"]
